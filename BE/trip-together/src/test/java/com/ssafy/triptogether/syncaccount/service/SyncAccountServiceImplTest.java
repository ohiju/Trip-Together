...
class SyncAccountServiceImplTest {
	...
	class BankAccountLoadTest {
		...
		void bankAccountLoad() {
			...
			given(twinkleBankClient.bankAccountsLoad(any(TwinkleBankAccountsLoadRequest.class))).willReturn(
				twinkleBankAccountsLoadResponse);
			// when
	...
	class AccountSyncTest {
		...
		void accountSyncInit() {
			// given
			given(twinkleBankClient.bankAccountsSync(any(TwinkleAccountSyncRequest.class))).willReturn(
				twinkleAccountSyncResponse);
			given(memberRepository.findById(anyLong())).willReturn(Optional.ofNullable(member));
